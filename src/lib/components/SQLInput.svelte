...
<form class="w-full relative overflow-hidden flex flex-col gap-2 ">
  <div class="flex flex-row z-50 py-2 items-center justify-between max-w-full w-full rounded-xl focus:outline-none">
    <input
    bind:value
    type="text"
    id={name}
    class="h-full w-[50%] max-w-full rounded-l-xl bg-white caret-berry-700 pl-3 focus:outline-none shadow"
    placeholder="SELECT * FROM products"
    />
    {@html github}
    <Highlight language={sql} code={value === "" ? "SELECT * FROM products" : value } class="h-full w-[50%] shadow " />  
