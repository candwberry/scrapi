<script lang="ts">
  ...
  const triggers = [
    { id: 'tab-1', title: 'Google' },
    { id: 'tab-2', title: 'Amazon' },
    { id: 'tab-3', title: 'eBay' },
  ];
      ...
      <button use:melt={$trigger(triggerItem.id)} class="trigger relative">
        ...
        {#if $value === triggerItem.id}
          <div
            ...
            class="absolute bottom-1 left-1/2 h-1 w-6 -translate-x-1/2 rounded-full bg-berry-400"
          />
        {/if}
      </button>
    {/each}
...
<style lang="postcss">
  ...
  input {
    ...
    &:focus {
      border-color: theme(colors.berry.400);
    }
  }
  ...
  .save {
    ...
    justify-content: center;
    border-radius: theme(borderRadius.md);
    background-color: theme(colors.berry.200);
