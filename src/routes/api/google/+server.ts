...
import { getDecentTime, initBrowser, extractDomain, consolelog, consoleerror, ok, err } from '$lib/utils';
...
const openai = new OpenAi({
    apiKey: Bun.env.OPEN_AI_API_KEY
});
...
async function findPrice(page: Page, validatedRegex?: string): Promise<{ price: string; priceFound: string; }> {
    ...
    if (price === "99999") {
        ...
        const emptyElements = Array.from(document.querySelectorAll('*')).filter(element => element.children.length === 0 && element.textContent.trim() === '');
        ...
        const largeTextElements = Array.from(document.querySelectorAll('*')).filter(element => element.children.length === 0 && element.textContent.length > 50);
        ...
        document.querySelectorAll('*').forEach((element) => {
            for (let i = element.childNodes.length - 1; i >= 0; i--) {
                ...
                if (child.nodeType === 8 || (child.nodeType === 3 && !/\S/.test(child.nodeValue))) { // 8 is comment, 3 is text, cool,.
...
export const GET: RequestHandler = async ({ request, url }) => {
    ...
    try {
            ...
            try {
                ...
                for (let i = 0; i < numBatches; i++) {
                    ...
                    const batchPromises = batchProducts.map(async (product: { barcode: string; title: string; berry: any; supplierCode: any; supplier: any; amazonLast: any; ebayLast: any; amazonJSON: any; }) => {
                        ...
                        } catch (error) { cerr(`Error processing product ${product.berry}:`, error); }
                    });
        ...
        return err("An error occurred while processing your request.", { error });
    } finally { if (browser) await browser.close().catch(err => cerr("Error closing browser:", err)); }
};
